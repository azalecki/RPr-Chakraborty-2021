...
tm_gee_residuals
```
```{r map-residualclusters}

study_states <- filter(gee_data, statefp == '12'  | statefp == '13' | statefp == '01' | statefp == '45') 

gee_resid_nogeo <- gee_resid %>% st_drop_geometry()

merged_study_states <- merge(x = study_states, y = gee_resid_nogeo, by = "ss_clusID", all = TRUE)

gee_resid_nogeo <- data.frame()

tm_gee_residuals <- tm_shape(merged_study_states) +
  tm_polygons("response",
    title = "Race Model Residuals",
    style = "equal",
    n = 7,
