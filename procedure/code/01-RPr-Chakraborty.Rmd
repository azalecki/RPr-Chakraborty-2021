...
author: "Joseph Holler, Junyi Zhou, Peter Kedron, Drew An-Pham, Derrick Burt, Alexandra Zalecki"
date: "`r Sys.Date()`"
...
Version 2.0 \| First Created July 7, 2021 \| Updated September 21, 2023
...
filter(acs_covid, is.na(bpov_pct)) %>% st_drop_geometry() %>% t() %>% kable(caption = "missing data")
...
```{r residuals}
# create data frame with residuals (for correct covid incidence)
...
# create data frame with residuals for r Geepack model 
gee_resid_gp <- data.frame(
  select(gee_or_clus_ordep$models[[1]]$data, covid_rate, ss_clusID),
  data.frame(fitvals = gee_or_clus_ordep$models[[1]]$fitted.values),
  data.frame(residual = gee_or_clus_ordep$models[[1]]$residual),
  data.frame(working = residuals(gee_or_clus_ordep$models[[1]], type = "working")),
  data.frame(response = residuals(gee_or_clus_ordep$models[[1]], type = "response")),
  data.frame(pearson = residuals(gee_or_clus_ordep$models[[1]], type = "pearson"))
) %>%
  mutate(
    my_working = (covid_rate - fitvals) / fitvals,
    my_pearson = response * (sqrt(gee_or_clus_ordep$models[[1]]$prior.weights)) / sqrt(fitvals^2)
  ) %>%
  st_sf()

# create data frame with residuals for sat scan gini optimized
gee_resid_gini <- data.frame(
  select(gee_gini_clus_rpdep$models[[1]]$data, covid_rate, ss_clusID),
  data.frame(fitvals = gee_gini_clus_rpdep$models[[1]]$fitted.values),
  data.frame(residual = gee_gini_clus_rpdep$models[[1]]$residual),
  data.frame(working = residuals(gee_gini_clus_rpdep$models[[1]], type = "working")),
  data.frame(response = residuals(gee_gini_clus_rpdep$models[[1]], type = "response")),
  data.frame(pearson = residuals(gee_gini_clus_rpdep$models[[1]], type = "pearson"))
) %>%
  mutate(
    my_working = (covid_rate - fitvals) / fitvals,
    my_pearson = response * (sqrt(gee_gini_clus_rpdep$models[[1]]$prior.weights)) / sqrt(fitvals^2)
  ) %>%
  st_sf()

# create data frame with residuals for Spatial EPI clusters model 
gee_resid_epi <- data.frame(
  select(gee_rp_clus_rpdep$models[[1]]$data, covid_rate, ss_clusID),
  ...
  mutate(
    my_working = (covid_rate - fitvals) / fitvals,
    my_pearson = response * (sqrt(gee_rp_clus_rpdep$models[[1]]$prior.weights)) / sqrt(fitvals^2)
  ) %>%
  st_sf()
...
```{r map-residuals}
# map of residuals from correct covid incidence model
tm_gee_residuals <- tm_shape(gee_resid) +
  tm_polygons("response",
    title = "Race Model Residuals- Correct Covid Inc",
    style = "equal",
    n = 7,
    border.alpha = .2,
    lwd = 0.2,
...
# map of residuals from geepack model
tm_gee_residuals_gp <- tm_shape(gee_resid_gp) +
  tm_polygons("response",
    title = "Race Model Residuals - Geepack Model",
    style = "equal",
    n = 7,
    border.alpha = .2,
    lwd = 0.2,
...
# map of residuals from sat scan gini optimized clusters 

tm_gee_residuals_gini <- tm_shape(gee_resid_gini) +
  tm_polygons("response",
    title = "Race Model Residuals- SatScan GINI",
    style = "equal",
    n = 7,
    border.alpha = .2,
    lwd = 0.2,
...
tm_gee_residuals_gini

# map of residuals from sat scan gini optimized clusters 

tm_gee_residuals_epi <- tm_shape(gee_resid_epi) +
  tm_polygons("response",
    title = "Race Model Residuals - Spatial EPI",
    style = "equal",
    n = 7,
    border.alpha = .2,
    lwd = 0.2,
...
tm_gee_residuals_epi

```

The 4 maps above illustrate the race model residuals of 4 model variations. Every model seems to produce varying differences between the observed and predicted values. Off the bat the SatScan GINI model produced results most consistent with observed values of the Covid-19 incidence and the race model. Across the United States, the Spatial EPI and Geepack Model produced results that generally overestimated rather than underestimated values. This especially the case in counties found in Southern states specifically in the East South Central and South Atlantic regions of the US. This was also the case in counties part of the Northeast and the Mountain West.
