...
tm_gee_residuals
```
```{r map-residualclusters-se}

geometry_data <- filter(gee_data, statefp == 06| statefp == 35 | statefp == 04 | statefp == 49)

<<<<<<< HEAD

intersection <- st_intersection(gee_resid, geometry_data)

tm_gee_residuals_se <- tm_shape(intersection) +
  tm_polygons("response",
    title = "Race Model Residuals",
    style = "equal",
    n = 7,
    ...
    midpoint = 0
=======
merged_study_states <- st_join(study_states, gee_resid, join = st_intersects, left = TRUE)

tm_gee_residuals_clusters <- 
  tm_shape(study_states) +
  tm_fill("gray98") +
  tm_shape(merged_study_states) +
  tm_polygons( col = "ss_clusID.x",
    labels = "gee clusters",
    border.col = "white",
    lwd = 0.2,
    palette = "Set 3"),
    legend.hist = FALSE
>>>>>>> 7c7c4fb542276f1d3829fda53f237780345d4104
  ) +
  tm_shape(state) +
    tm_borders("grey", lwd = .5) +
  tm_layout(
